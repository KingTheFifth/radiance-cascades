    ...
    fn init(gl: &glow::Context, _window: &sdl2::video::Window) -> Self {
        ...
        unsafe {
            ...
            let program = load_shaders(
                gl,
                include_str!("colored.vert"),
                include_str!("colored.frag"),
            );
            ...
            let color_loc = gl.get_attrib_location(program, "in_Color").unwrap();
...
fn main() {
    Demo::sdl2_window("Shaded tringle example").start();
}
